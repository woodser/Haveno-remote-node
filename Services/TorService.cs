...
public class TorService
{
    ...
    private static async Task<string?> EnsureMacTorInstalledAsync()
    {
        ...
        try
        {
            using var downloads = JsonDocument.Parse(await httpClient.GetStringAsync("https://aus1.torproject.org/torbrowser/update_3/release/downloads.json"));
        ...
        try
        {
            ...
            {
                var codesign = Process.Start(new ProcessStartInfo { FileName = "/usr/bin/codesign", Arguments = $"-f -s - \"{file}\"", UseShellExecute = false });
